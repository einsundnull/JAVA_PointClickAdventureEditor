...
public class SceneReferenceManager {
	...
	public static String getSceneBackgroundImage(String sceneName) throws IOException {
		...
		boolean foundBackgroundImagesSection = false;
		boolean inImageBlock = false;

		for (String line : lines) {
			String trimmed = line.trim();

			// Check for new format: #BackgroundImages:
			if (trimmed.equals("#BackgroundImages:")) {
				foundBackgroundImagesSection = true;
				continue;
			}

			// If in BackgroundImages section, look for first image
			if (foundBackgroundImagesSection) {
				if (trimmed.equals("-Image:")) {
					inImageBlock = true;
					continue;
				}

				// Get path from first image
				if (inImageBlock && trimmed.startsWith("--Path:")) {
					return trimmed.substring(7).trim(); // Remove "--Path:"
				}

				// Stop if we hit another # section
				if (trimmed.startsWith("#") && !trimmed.equals("#BackgroundImages:")) {
					break;
				}
			}

			// Check for old format: #Backgroundimage: (for backwards compatibility)
			if (trimmed.equals("#Backgroundimage:")) {
				foundBackgroundSection = true;
				continue;
			}

			// If we found the old section, next line starting with - is the image
			if (foundBackgroundSection && trimmed.startsWith("-")) {
				return trimmed.substring(1).trim(); // Remove leading dash
			}

			// If we hit another # section in old format, stop looking
			if (foundBackgroundSection && trimmed.startsWith("#")) {
				break;
			}
		}
