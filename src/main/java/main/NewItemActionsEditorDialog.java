...
public class NewItemActionsEditorDialog extends JDialog {
    ...
    public NewItemActionsEditorDialog(Frame owner, Item item, String actionName) {
        ...
        System.out.println("=== NewItemActionsEditorDialog CONSTRUCTOR ===");
        ...
        System.out.println("=== NewItemActionsEditorDialog READY ===");
    }
    ...
    private void initUI() {
        ...
        JButton processEditorBtn = new JButton("⚡ Process Editor (Advanced)");
        processEditorBtn.setFont(new Font("Arial", Font.BOLD, 14));
        processEditorBtn.setForeground(new Color(0, 100, 200));
        processEditorBtn.addActionListener(e -> {
            System.out.println("=== PROCESS EDITOR BUTTON CLICKED ===");
            openProcessEditor();
        });
        topPanel.add(processEditorBtn);
        System.out.println("NewItemActionsEditorDialog: Process Editor button added to UI");
    ...
    private void saveActions() {
        ...
        try {
            ...
            // Save the entire item to file (DEFAULT)
            ItemSaver.saveItemToDefault(item);
            System.out.println("Item saved to DEFAULT file");
            ...
            try {
                ...
                if (owner instanceof EditorMain) {
                    ...
                    // Replace the item in the scene's item list
                    List<Item> items = currentScene.getItems();
                    for (int i = 0; i < items.size(); i++) {
                        if (items.get(i).getName().equals(itemName)) {
                            items.set(i, reloadedItem);
                            System.out.println("Item reloaded and replaced in scene");
                            break;
                        }
                    }
                }
            } catch (Exception reloadEx) {
                System.err.println("Warning: Could not reload item after save: " + reloadEx.getMessage());
            }

            System.out.println("=== SAVE COMPLETE ===");

            JOptionPane.showMessageDialog(this, "Actions saved successfully to " + item.getName() + ".txt!\n\nItem reloaded from file.", "Saved",
    ...
    private void openProcessEditor() {
        System.out.println("=== openProcessEditor() CALLED ===");
        ...
        try {
            if (getOwner() instanceof EditorMain) {
                System.out.println("Owner is EditorMain - creating ProcessEditorDialog");
                EditorMain editorWindow = (EditorMain) getOwner();
                ProcessEditorDialog dialog = new ProcessEditorDialog(editorWindow, item, actionName);
                System.out.println("ProcessEditorDialog created, showing...");
                dialog.setVisible(true);

                System.out.println("ProcessEditorDialog closed, reloading actions");
                // After closing the process editor, reload the current action data
                loadExistingActions();
            } else {
                System.out.println("ERROR: Owner is not EditorMain, it is: " + getOwner().getClass().getName());
            }
        } catch (Exception ex) {
            System.err.println("ERROR in openProcessEditor: " + ex.getMessage());
            ex.printStackTrace();
        }
    }
