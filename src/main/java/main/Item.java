...
public class Item {
    ...
    private List<MovingRange> movingRanges; // Multiple moving ranges with conditions
    private List<Path> paths; // Multiple moving ranges with conditions
    ...
    public Item(String name) {
        ...
        this.movingRanges = new ArrayList<>();
    ...
    public void addMovingRange(MovingRange range) {
        this.movingRanges.add(range);
    }

    public MovingRange getPrimaryMovingRange() {
        if (movingRanges == null || movingRanges.isEmpty()) {
            return null;
        }
        return movingRanges.get(0);
    }

    public MovingRange ensurePrimaryMovingRange() {
        if (movingRanges == null) {
            movingRanges = new ArrayList<>();
        }
        if (movingRanges.isEmpty() || movingRanges.get(0) == null) {
            MovingRange range = new MovingRange();
            if (movingRanges.isEmpty()) {
                movingRanges.add(range);
            } else {
                movingRanges.set(0, range);
            }
        }
        return movingRanges.get(0);
    }
    ...
    public void removeMovingRange(MovingRange range) {
        this.movingRanges.remove(range);
    }
    ...
    public void consolidatePointContainers() {
        ...
        if (movingRanges != null && movingRanges.size() > 1) {
            MovingRange primaryRange = ensurePrimaryMovingRange();
            for (int i = 1; i < movingRanges.size(); i++) {
                MovingRange extra = movingRanges.get(i);
                mergePointData(primaryRange, extra);
            }
            movingRanges.subList(1, movingRanges.size()).clear();
        }
