...
public class ItemActionsEditorDialog extends JDialog {
	...
	public ItemActionsEditorDialog(EditorMain parent, Item item) {
	...
	private void initUI() {
		...
		JButton processEditorBtn = new JButton("⚡ Process Editor");
		processEditorBtn.setFont(new Font("Arial", Font.BOLD, 14));
		processEditorBtn.addActionListener(e -> openProcessEditorForCurrentTab());
		leftButtons.add(processEditorBtn);
	...
	private JPanel createActionPanel(String actionName) {
		...
		openEditorBtn.addActionListener(e -> {
			NewItemActionsEditorDialog dialog = new NewItemActionsEditorDialog(parent, item, actionName);
	...
	private void saveAllActions() {
		parent.log("Saving actions for " + item.getName() + "...");
		...
		for (int i = 0; i < tabbedPane.getTabCount(); i++) {
			...
			if (comp instanceof JPanel) {
				...
				// Note: Other actions are now handled by NewItemActionsEditorDialog
				// which saves directly to resources/actions/<itemName>.txt
			}
		}
	...
	private void openHoverEditor() {
		...
		HoverEditorDialog dialog = new HoverEditorDialog(parent, item);
		dialog.setVisible(true);
	...
	private void openProcessEditorForCurrentTab() {
		try {
			// Get currently selected tab
			int selectedIndex = tabbedPane.getSelectedIndex();
			parent.log("DEBUG: Selected tab index: " + selectedIndex);

			if (selectedIndex < 0) {
				parent.log("ERROR: No tab selected!");
				return;
			}

			String actionName = tabbedPane.getTitleAt(selectedIndex);
			parent.log("Opening Process Editor for: " + item.getName() + " - " + actionName);

			ProcessEditorDialog dialog = new ProcessEditorDialog(parent, item, actionName);
			parent.log("DEBUG: ProcessEditorDialog created successfully");

			dialog.setVisible(true);
			parent.log("Process Editor closed");

		} catch (Exception e) {
			parent.log("ERROR opening Process Editor: " + e.getMessage());
			e.printStackTrace();
