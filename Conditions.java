...
public class Conditions {
    ...
    // Mode: true = gaming mode (save to progress), false = edit mode (save to default)
    private static boolean gamingMode = false;

    // Auto-save to progress enabled/disabled (deprecated - use gamingMode instead)
    @Deprecated
    private static boolean autoSaveToProgress = false;
    ...
    /**
     * Set gaming mode (true) or edit mode (false)
     * Gaming mode: save to conditions_progress.txt
     * Edit mode: save to conditions.txt (defaults)
     */
    public static void setGamingMode(boolean enabled) {
        gamingMode = enabled;
        System.out.println("Conditions mode: " + (enabled ? "GAMING MODE (save to progress)" : "EDIT MODE (save to default)"));
    }

    /**
     * Check if currently in gaming mode
     */
    public static boolean isGamingMode() {
        return gamingMode;
    }

    /**
     * @deprecated Use setGamingMode() instead
     */
    @Deprecated
    public static void setAutoSaveToProgress(boolean enabled) {
        autoSaveToProgress = enabled;
        gamingMode = enabled; // For backward compatibility
        System.out.println("Auto-save to progress: " + (enabled ? "ENABLED" : "DISABLED"));
    }
    ...
    public static void setCondition(String name, boolean value) {
        ...
        if (changed) {
            if (gamingMode) {
                saveConditionsToProgress();
                System.out.println("  → Auto-saved to progress (gaming mode)");
            } else if (autoSaveToProgress) {
                // Legacy support
                saveConditionsToProgress();
                System.out.println("  → Auto-saved to progress (legacy)");
            }
            // Note: In edit mode, changes are not auto-saved to default - must be saved explicitly
        }
    }
    ...
    /**
     * Speichert alle Conditions in conditions.txt (DEFAULT VALUES)
     */
    ...
    /**
     * Speichert alle Conditions in conditions/conditions.txt (CURRENT STATE)
     * Used for auto-save in Gaming Mode
     */
    public static void saveConditionsToProgress() {
        try {
            ...
            BufferedWriter writer = new BufferedWriter(new FileWriter(file));
            writer.write("# Conditions Current State File\n");
            writer.write("# Format: conditionName = currentValue\n");
            writer.write("# This file stores the current game state\n\n");
            ...
            writer.close();
            System.out.println("✓ Conditions saved to: resources/conditions/conditions.txt (CURRENT STATE)");
        } catch (Exception e) {
            System.err.println("ERROR saving conditions to current state: " + e.getMessage());
        }
    }
    ...
    /**
     * Lädt Conditions aus den DEFAULT-Dateien (für Editor-Modus)
    ...
    /**
     * Speichert alle Conditions in die DEFAULT-Datei (für Editor-Modus)
